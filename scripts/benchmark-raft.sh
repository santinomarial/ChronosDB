...
if (($# < 1)); then
  echo "usage: $0 OUTPUT_DIRECTORY [chronos-raftbench options]" >&2
  exit 2
fi
...
raftbench="${repo_root}/build/raft-benchmark/chronos-raftbench"
temporary_log="$(mktemp "${output_parent}/.chronos-raftbench-log.XXXXXX")"
cleanup_log() {
...
mv -- "${temporary_log}" "${output_directory}/raftbench.log"
...
cp -- "${repo_root}/build/raft-benchmark/CMakeCache.txt" "${output_directory}/CMakeCache.txt"
cp -- "${repo_root}/build/raft-benchmark/compile_commands.json" \
