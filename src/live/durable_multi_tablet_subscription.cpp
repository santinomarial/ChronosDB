...
common::Result<MultiTabletSnapshotSubscription>
DurableMultiTabletSubscription::start_mixed_snapshot(
    const PreparedSubscriptionPlan& plan, const common::Uuid subscription_id,
    const query::QueryResourceContext& resources, const manifest::ManifestStorage& storage,
  ...
  try {
    std::vector<SnapshotSubscriptionColumn> columns{plan.columns().begin(), plan.columns().end()};
    return MultiTabletSnapshotSubscription::start_mixed(
        impl_->manager, plan.request(subscription_id), resources, storage, publisher, application,
        lineage, plan.schema_ptr()->schema_id(), plan.physical_plan(), std::move(columns), limits);
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(
        exhausted("durable mixed subscription snapshot allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(
        exhausted("durable mixed subscription snapshot exceeds container limits"));
  }
}
