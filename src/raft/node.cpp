...
public:
  ...
  [[nodiscard]] common::Result<Transition> append_local_entry(const std::uint8_t type,
...
common::Result<Transition> RaftNode::receive(const NodeId source, Message message) {
  ...
  if (request_vote_request || append_entries_request || install_snapshot_request ||
    ...
    try {
      ...
      return common::make_unexpected(
          exhausted("Raft receive transition preparation allocation failed"));
    } catch (const std::length_error&) {
      return common::make_unexpected(
...
common::Result<Transition> RaftNode::propose(const std::uint8_t type,
  ...
  try {
    Impl prepared = *impl_;
    auto appended = prepared.append_local_entry(type, std::move(payload));
    if (!appended.has_value())
...
common::Result<Transition> RaftNode::commit_current_term() {
  ...
  try {
    Impl prepared = *impl_;
    auto appended = prepared.append_local_entry(kLeaderNoopEntryType, {});
    if (!appended.has_value())
      return common::make_unexpected(appended.error());
    Transition transition = std::move(*appended);
    ...
    *impl_ = std::move(prepared);
    ...
    return common::make_unexpected(exhausted("Raft current-term progress allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(
        exhausted("Raft current-term progress exceeds container limits"));
  }
}
...
common::Result<Transition> RaftNode::compact_snapshot(SnapshotMetadata snapshot) {
  ...
  try {
    ...
    return common::make_unexpected(exhausted("Raft snapshot compaction allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(exhausted("Raft snapshot compaction exceeds container limits"));
  }
}
