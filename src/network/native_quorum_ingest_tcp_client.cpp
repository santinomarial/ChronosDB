...
[[nodiscard]] bool valid_timeout(const std::chrono::milliseconds timeout) noexcept {
  const auto maximum = std::chrono::duration_cast<std::chrono::milliseconds>(
      NativeQuorumIngestTcpClient::TimePoint::duration::max());
  return timeout.count() > 0 && timeout <= maximum;
}

[[nodiscard]] NativeQuorumIngestTcpClient::TimePoint
deadline_after(const NativeQuorumIngestTcpClient::TimePoint now,
               const std::chrono::milliseconds timeout) noexcept {
  const auto duration =
      std::chrono::duration_cast<NativeQuorumIngestTcpClient::TimePoint::duration>(timeout);
  return now > NativeQuorumIngestTcpClient::TimePoint::max() - duration
             ? NativeQuorumIngestTcpClient::TimePoint::max()
             : now + duration;
}

} // namespace

class NativeQuorumIngestTcpClient::Impl {
public:
  Impl(TcpSocket owned_socket, NativeQuorumIngestRetry owned_retry,
       ...
       const NativeQuorumIngestTcpClientLimits configured_limits, const TimePoint now) noexcept
  ...
  [[nodiscard]] common::Status fail(common::Status failure) {
    if (client_state != NativeQuorumIngestTcpClientState::kFailed) {
      tls.reset();
      static_cast<void>(socket.close());
      client_failure = std::move(failure);
      client_state = NativeQuorumIngestTcpClientState::kFailed;
      readiness = {};
    }
  ...
  void complete() {
    tls.reset();
    static_cast<void>(socket.close());
    client_state = NativeQuorumIngestTcpClientState::kComplete;
    readiness = {};
  }
  ...
  [[nodiscard]] common::Status authenticate_server(const TimePoint now) {
    ...
    if (active_tls == nullptr)
      return fail(
          status(common::StatusCode::kInternal, "native QUORUM_SYNC TLS session is missing"));
    ...
    if (!authentication->authorized || authentication->principal_id == 0U)
      return fail(status(common::StatusCode::kUnauthenticated,
                         "native QUORUM_SYNC server principal is not authenticated"));
    ...
    if (!*authorized)
      return fail(status(common::StatusCode::kUnauthenticated,
                         "native QUORUM_SYNC server principal cannot claim the route node"));
    client_state = NativeQuorumIngestTcpClientState::kExchanging;
  ...
  [[nodiscard]] common::Status begin_redirect_connect(const TimePoint now) {
    ...
    client_state = NativeQuorumIngestTcpClientState::kConnecting;
    active_deadline = deadline_after(now, limits.connect_timeout);
    readiness = {.want_write = true};
    return common::Status::ok();
  }
  ...
  [[nodiscard]] common::Status advance_connect(const bool writable, const TimePoint now) {
    ...
    client_state = NativeQuorumIngestTcpClientState::kHandshaking;
  ...
  [[nodiscard]] common::Status advance_handshake(const bool readable, const bool writable,
    ...
    if (progress->state == TlsIoState::kClosed)
      return fail(
          status(common::StatusCode::kUnavailable, "native QUORUM_SYNC TLS handshake closed"));
  ...
  [[nodiscard]] common::Status advance_write(const bool readable, const bool writable) {
    ...
    if (progress->state == TlsIoState::kClosed)
      return fail(
          status(common::StatusCode::kUnavailable, "native QUORUM_SYNC request socket closed"));
    if (progress->state == TlsIoState::kWantRead) {
      readiness = {.want_read = true};
      return common::Status::ok();
    }
    ...
    if (progress->bytes_transferred == 0U)
      return fail(status(common::StatusCode::kUnavailable,
                         "native QUORUM_SYNC request write made no progress"));
    const common::Status consumed = retry.consume_written(progress->bytes_transferred);
    if (!consumed.is_ok())
      return fail(consumed);
    readiness = retry.pending_write().empty() ? NativeQuorumIngestTcpInterest{.want_read = true}
                                              : NativeQuorumIngestTcpInterest{.want_write = true};
    return common::Status::ok();
  }
  ...
  [[nodiscard]] common::Status advance_read(const bool readable, const bool writable,
    ...
    if (progress->state == TlsIoState::kClosed)
      return fail(
          status(common::StatusCode::kUnavailable, "native QUORUM_SYNC response socket closed"));
    if (progress->state == TlsIoState::kWantRead) {
      readiness = {.want_read = true};
      return common::Status::ok();
    }
  ...
  NativeQuorumIngestTcpClientState client_state{NativeQuorumIngestTcpClientState::kConnecting};
  NativeQuorumIngestTcpInterest readiness{.want_write = true};
  common::Status client_failure{common::StatusCode::kInternal,
                                "native QUORUM_SYNC TCP client has not failed"};
};

NativeQuorumIngestTcpClient::NativeQuorumIngestTcpClient(
    std::unique_ptr<Impl> implementation) noexcept
    : implementation_(std::move(implementation)) {}
NativeQuorumIngestTcpClient::~NativeQuorumIngestTcpClient() = default;
NativeQuorumIngestTcpClient::NativeQuorumIngestTcpClient(NativeQuorumIngestTcpClient&&) noexcept =
    default;
NativeQuorumIngestTcpClient&
NativeQuorumIngestTcpClient::operator=(NativeQuorumIngestTcpClient&&) noexcept = default;

common::Result<NativeQuorumIngestTcpClient>
NativeQuorumIngestTcpClient::begin(NativeQuorumIngestTcpClientConfig config,
  ...
  if (config.authenticator == nullptr || config.node_authorizer == nullptr ||
    ...
    return common::make_unexpected(status(common::StatusCode::kInvalidArgument,
                                          "native QUORUM_SYNC TCP configuration is invalid"));
  }
  auto retry =
      NativeQuorumIngestRetry::create(std::move(config.retry), std::move(encoded_columnar_append));
  if (!retry.has_value())
  ...
  try {
    ...
    return NativeQuorumIngestTcpClient{
    ...
    return common::make_unexpected(
        status(common::StatusCode::kResourceExhausted, "native QUORUM_SYNC TCP allocation failed"));
  }
}

common::Status NativeQuorumIngestTcpClient::on_ready(const bool readable, const bool writable,
                                                     const TimePoint now) {
  if (!implementation_)
    return status(common::StatusCode::kInvalidArgument, "native QUORUM_SYNC TCP client is empty");
  Impl& impl = *implementation_;
  if (impl.client_state == NativeQuorumIngestTcpClientState::kFailed)
    return impl.client_failure;
  if (impl.client_state == NativeQuorumIngestTcpClientState::kComplete)
    return common::Status::ok();
  if (now >= impl.active_deadline) {
    const char* message = impl.client_state == NativeQuorumIngestTcpClientState::kConnecting
                              ? "native QUORUM_SYNC TCP connect timed out"
                          : impl.client_state == NativeQuorumIngestTcpClientState::kHandshaking
                              ? "native QUORUM_SYNC TLS handshake timed out"
                              : "native QUORUM_SYNC exchange timed out";
    return impl.fail(status(common::StatusCode::kUnavailable, message));
  }
  if (impl.client_state == NativeQuorumIngestTcpClientState::kConnecting)
    return impl.advance_connect(writable, now);
  if (impl.client_state == NativeQuorumIngestTcpClientState::kHandshaking)
...
NativeQuorumIngestTcpClientState NativeQuorumIngestTcpClient::state() const noexcept {
  return implementation_ ? implementation_->client_state
                         : NativeQuorumIngestTcpClientState::kFailed;
}

NativeQuorumIngestTcpInterest NativeQuorumIngestTcpClient::interest() const noexcept {
  return implementation_ ? implementation_->readiness : NativeQuorumIngestTcpInterest{};
}

int NativeQuorumIngestTcpClient::descriptor() const noexcept {
  return implementation_ ? implementation_->socket.descriptor() : -1;
}

std::optional<NativeQuorumIngestTcpClient::TimePoint>
NativeQuorumIngestTcpClient::deadline() const noexcept {
  if (!implementation_ ||
      implementation_->client_state == NativeQuorumIngestTcpClientState::kFailed ||
      implementation_->client_state == NativeQuorumIngestTcpClientState::kComplete) {
    return std::nullopt;
  }
  return implementation_->active_deadline;
}

NativeLeaderRoute NativeQuorumIngestTcpClient::current_route() const noexcept {
  return implementation_ ? implementation_->retry.current_route() : NativeLeaderRoute{};
}

std::size_t NativeQuorumIngestTcpClient::attempts_started() const noexcept {
  return implementation_ ? implementation_->retry.attempts_started() : 0U;
}

common::Result<QuorumSyncIngestAcknowledgement> NativeQuorumIngestTcpClient::result() const {
  if (!implementation_ ||
      implementation_->client_state != NativeQuorumIngestTcpClientState::kComplete) {
    return common::make_unexpected(status(common::StatusCode::kInvalidArgument,
                                          "native QUORUM_SYNC TCP result is unavailable"));
  }
  return implementation_->retry.result();
}

const common::Status& NativeQuorumIngestTcpClient::failure() const noexcept {
  static const common::Status empty{common::StatusCode::kInvalidArgument,
                                    "native QUORUM_SYNC TCP client is empty"};
