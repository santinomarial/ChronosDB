...
public:
  ...
  void evict_for(const std::size_t bytes) {
    while (!recency.empty() &&
           (cache.size() >= limits.maximum_cache_entries ||
...
TieredPartManager::TieredPartManager(std::unique_ptr<Impl> impl) noexcept : impl_(std::move(impl)) {}
...
common::Result<TieringReceipt> TieredPartManager::upload_and_install(
  ...
  if (!checksum.has_value()) return common::make_unexpected(checksum.error());
  ...
  if (impl_->parts.size() >= impl_->limits.maximum_parts) {
    return common::make_unexpected(common::Status{common::StatusCode::kResourceExhausted,
                                                   "cold part catalog is full"});
  }
  auto uploaded = impl_->store->put_if_absent(descriptor.object_key, local_cseg, descriptor.checksum);
  if (!uploaded.has_value()) return common::make_unexpected(uploaded.error());
  auto verified = impl_->store->stat(descriptor.object_key);
  if (!verified.has_value()) return common::make_unexpected(verified.error());
  ...
  if (!installed.is_ok()) return common::make_unexpected(installed);
  impl_->parts.emplace(descriptor.part.part_id, descriptor);
  return TieringReceipt{std::move(descriptor), true};
}

common::Result<std::vector<std::byte>> TieredPartManager::read_range(
    const cseg::PartId& part_id, const std::size_t offset, const std::size_t length,
  ...
  if (cached != impl_->cache.end()) {
    ...
    return std::vector<std::byte>{cached->second.bytes.begin() + static_cast<std::ptrdiff_t>(offset),
  ...
  if (size <= impl_->limits.maximum_cache_bytes) {
    ...
    if (!full.has_value()) return common::make_unexpected(full.error());
    auto checksum = ingest::sha256(*full);
    if (!checksum.has_value()) return common::make_unexpected(checksum.error());
    if (*checksum != part->second.checksum) {
      return common::make_unexpected(common::Status{common::StatusCode::kCorruption,
                                                     "cold object checksum mismatch"});
    }
    impl_->evict_for(size);
    impl_->recency.push_front(part_id);
    impl_->cache_bytes += size;
    auto [entry, inserted] = impl_->cache.emplace(
        part_id, Impl::CacheEntry{std::move(*full), impl_->recency.begin()});
...
std::size_t TieredPartManager::cached_bytes() const noexcept { return impl_->cache_bytes; }
std::size_t TieredPartManager::cached_entries() const noexcept { return impl_->cache.size(); }
