...
[[nodiscard]] common::Status poll_error(const int error = errno) {
  return {common::StatusCode::kIoError,
          std::string("polling grouped shuffle result TCP server: ") +
              std::error_code(error, std::generic_category()).message()};
}

[[nodiscard]] bool valid_timeout(const std::chrono::milliseconds timeout) noexcept {
  const auto maximum = std::chrono::duration_cast<std::chrono::milliseconds>(
      DistributedVectorGroupedAggregateShuffleResultTlsServer::TimePoint::duration::max());
...
class DistributedVectorGroupedAggregateShuffleResultTcpServer::Impl {
public:
  struct Connection {
    Connection(network::TcpSocket socket,
               DistributedVectorGroupedAggregateShuffleResultTlsServer carrier,
    ...
    DistributedVectorGroupedAggregateShuffleResultTlsServer carrier_;
    std::size_t completion_slot_{};
  };

  Impl(DistributedVectorGroupedAggregateShuffleResultTcpServerConfig config,
    ...
    for (std::size_t admitted = 0U; admitted < config_.maximum_accepts_per_poll; ++admitted) {
      ...
      auto carrier = DistributedVectorGroupedAggregateShuffleResultTlsServer::create(
          ...
          {.authenticator = config_.authenticator,
           ...
           .result_schema = config_.result_schema,
  ...
  DistributedVectorGroupedAggregateShuffleResultTcpServerConfig config_;
  ...
  DistributedVectorGroupedAggregateShuffleResultTcpServerMetrics metrics_;
  bool running_{true};
};

DistributedVectorGroupedAggregateShuffleResultTcpServer::
    DistributedVectorGroupedAggregateShuffleResultTcpServer() noexcept = default;
DistributedVectorGroupedAggregateShuffleResultTcpServer::
    DistributedVectorGroupedAggregateShuffleResultTcpServer(
        std::unique_ptr<Impl> implementation) noexcept
    : implementation_(std::move(implementation)) {}
DistributedVectorGroupedAggregateShuffleResultTcpServer::
    ~DistributedVectorGroupedAggregateShuffleResultTcpServer() = default;
DistributedVectorGroupedAggregateShuffleResultTcpServer::
    DistributedVectorGroupedAggregateShuffleResultTcpServer(
        DistributedVectorGroupedAggregateShuffleResultTcpServer&&) noexcept = default;
DistributedVectorGroupedAggregateShuffleResultTcpServer&
DistributedVectorGroupedAggregateShuffleResultTcpServer::operator=(
    DistributedVectorGroupedAggregateShuffleResultTcpServer&&) noexcept = default;

common::Result<DistributedVectorGroupedAggregateShuffleResultTcpServer>
DistributedVectorGroupedAggregateShuffleResultTcpServer::start(
  ...
  if (config.authenticator == nullptr || config.node_authorizer == nullptr ||
      ...
      config.coordinator_node_id == 0U || config.maximum_retained_streams == 0U ||
      config.maximum_retained_streams > 65536U || config.maximum_accepts_per_poll == 0U ||
      config.maximum_accepts_per_poll > 1024U || !valid_limits(config.carrier_limits) ||
      !query::validate_distributed_vector_result_schema_value(*config.result_schema).is_ok()) {
    return common::make_unexpected(
        invalid("grouped shuffle result TCP server configuration is invalid"));
  }
  ...
  try {
    ...
    std::vector<std::optional<DistributedVectorGroupedAggregateShuffleCompleteResultStream>> slots(
    ...
    return DistributedVectorGroupedAggregateShuffleResultTcpServer{
    ...
    return common::make_unexpected(
        exhausted("grouped shuffle result TCP server allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(
        exhausted("grouped shuffle result TCP server limits are too large"));
  }
}

common::Status DistributedVectorGroupedAggregateShuffleResultTcpServer::poll_once(
    const std::chrono::milliseconds maximum_wait) {
  if (!implementation_ || !implementation_->running_)
    return invalid("grouped shuffle result TCP server is not running");
  ...
  for (std::size_t remaining = impl.connections_.size(); remaining > 0U; --remaining) {
    ...
    if (!status.is_ok() ||
        state == DistributedVectorGroupedAggregateShuffleResultTlsState::kFailed) {
      impl.remove_failed_connection(index);
    } else if (state == DistributedVectorGroupedAggregateShuffleResultTlsState::kComplete) {
...
DistributedVectorGroupedAggregateShuffleResultTcpServer::take_next_complete_stream() {
  if (!implementation_)
    return common::make_unexpected(invalid("grouped shuffle result TCP server is empty"));
  Impl& impl = *implementation_;
  if (impl.completed_count_ == 0U) {
    return common::make_unexpected(
        unavailable("grouped shuffle result TCP complete stream is unavailable"));
  }
...
common::Status DistributedVectorGroupedAggregateShuffleResultTcpServer::shutdown() {
  if (!implementation_ || !implementation_->running_)
    return common::Status::ok();
  Impl& impl = *implementation_;
...
DistributedVectorGroupedAggregateShuffleResultTcpServer::bound_endpoint() const noexcept {
  return implementation_ ? implementation_->listener_.bound_endpoint() : network::Ipv4Endpoint{};
}

DistributedVectorGroupedAggregateShuffleResultTcpServerMetrics
DistributedVectorGroupedAggregateShuffleResultTcpServer::metrics() const noexcept {
  return implementation_ ? implementation_->metrics_
                         : DistributedVectorGroupedAggregateShuffleResultTcpServerMetrics{};
}

bool DistributedVectorGroupedAggregateShuffleResultTcpServer::is_running() const noexcept {
