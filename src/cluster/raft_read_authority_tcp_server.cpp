...
[[nodiscard]] common::Status poll_error(const int error = errno) {
  return {common::StatusCode::kIoError,
          std::string("polling Raft read-authority TCP server: ") +
...
class RaftReadAuthorityTcpServer::Impl {
public:
  struct Connection {
    Connection(network::TcpSocket owned_socket, RaftReadAuthorityTlsServer owned_session) noexcept
    ...
    RaftReadAuthorityTlsServer session;
  };

  Impl(RaftReadAuthorityTcpServerConfig configured, network::TcpListener listener_owner,
  ...
  void accept_ready(const std::chrono::steady_clock::time_point now) {
    for (std::size_t accepted = 0U; accepted < config.maximum_accepts_per_poll; ++accepted) {
      ...
      auto session = RaftReadAuthorityTlsServer::create(std::move(*tls),
  ...
  RaftReadAuthorityTcpServerConfig config;
  network::TcpListener listener;
  network::TlsServerContext tls_context;
  std::vector<std::unique_ptr<Connection>> connections;
  std::vector<pollfd> poll_descriptors;
  RaftReadAuthorityTcpServerMetrics server_metrics;
  bool running{true};
};

RaftReadAuthorityTcpServer::RaftReadAuthorityTcpServer() noexcept = default;
RaftReadAuthorityTcpServer::RaftReadAuthorityTcpServer(
    std::unique_ptr<Impl> implementation) noexcept
    : implementation_(std::move(implementation)) {}
RaftReadAuthorityTcpServer::~RaftReadAuthorityTcpServer() = default;
RaftReadAuthorityTcpServer::RaftReadAuthorityTcpServer(RaftReadAuthorityTcpServer&&) noexcept =
    default;
RaftReadAuthorityTcpServer&
RaftReadAuthorityTcpServer::operator=(RaftReadAuthorityTcpServer&&) noexcept = default;

common::Result<RaftReadAuthorityTcpServer>
RaftReadAuthorityTcpServer::start(RaftReadAuthorityTcpServerConfig config) {
  if (config.authenticator == nullptr || config.receiver == nullptr ||
    ...
    return common::make_unexpected(
        status(common::StatusCode::kInvalidArgument,
               "Raft read-authority TCP server configuration is invalid"));
  }
  auto limits = RaftReadAuthorityResponseReader::create(config.session_limits.transport);
  if (!limits.has_value())
  ...
  try {
    ...
    return RaftReadAuthorityTcpServer{
    ...
    return common::make_unexpected(status(common::StatusCode::kResourceExhausted,
                                          "Raft read-authority TCP server allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(status(common::StatusCode::kResourceExhausted,
                                          "Raft read-authority TCP server limits are too large"));
  }
}

common::Status RaftReadAuthorityTcpServer::poll_once(const std::chrono::milliseconds maximum_wait) {
  if (!implementation_ || !implementation_->running) {
    return status(common::StatusCode::kInvalidArgument,
                  "Raft read-authority TCP server is not running");
  }
  if (maximum_wait.count() < 0 || maximum_wait.count() > INT_MAX) {
    return status(common::StatusCode::kInvalidArgument,
                  "Raft read-authority TCP poll timeout is invalid");
  }
  ...
  for (std::size_t remaining = impl.connections.size(); remaining > 0U; --remaining) {
    ...
    if (!progress.is_ok() || state == RaftReadAuthorityTlsServerState::kFailed)
      impl.remove_connection(index, false);
    else if (state == RaftReadAuthorityTlsServerState::kComplete)
...
common::Status RaftReadAuthorityTcpServer::shutdown() {
  if (!implementation_ || !implementation_->running)
    return common::Status::ok();
  Impl& impl = *implementation_;
...
network::Ipv4Endpoint RaftReadAuthorityTcpServer::bound_endpoint() const noexcept {
  return implementation_ ? implementation_->listener.bound_endpoint() : network::Ipv4Endpoint{};
}

RaftReadAuthorityTcpServerMetrics RaftReadAuthorityTcpServer::metrics() const noexcept {
  return implementation_ ? implementation_->server_metrics : RaftReadAuthorityTcpServerMetrics{};
}

bool RaftReadAuthorityTcpServer::is_running() const noexcept {
