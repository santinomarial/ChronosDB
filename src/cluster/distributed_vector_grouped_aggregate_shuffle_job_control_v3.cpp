...
using namespace distributed_vector_grouped_aggregate_shuffle_job_control_v3_format;
...
validate_cancel(const DistributedVectorGroupedAggregateShuffleJobCancel& request) {
  if (request.query_id.is_nil() || request.coordinator_node_id == 0U ||
    ...
    return invalid("grouped shuffle reducer-job cancellation is invalid");
  }
...
[[nodiscard]] common::Result<std::uint8_t> encode_status(const common::StatusCode code) {
  ...
  if (value > static_cast<std::uint8_t>(common::StatusCode::kInternal))
    return common::make_unexpected(invalid("grouped shuffle cancellation status is invalid"));
  return value;
}
...
[[nodiscard]] common::Result<common::StatusCode> decode_status(const std::uint8_t value) {
  if (value > static_cast<std::uint8_t>(common::StatusCode::kInternal)) {
    return common::make_unexpected(
        corruption("grouped shuffle cancellation response status is unknown"));
  }
...
encode_distributed_vector_grouped_aggregate_shuffle_job_cancel_v3(
    const DistributedVectorGroupedAggregateShuffleJobCancel& request) {
  const common::Status valid = validate_cancel(request);
  if (!valid.is_ok())
  ...
  try {
    ...
    if (!write.is_ok() || !writer.full())
      return common::make_unexpected(invalid("grouped shuffle cancellation frame layout failed"));
    return EncodedDistributedVectorGroupedAggregateShuffleJobControlRequest{std::move(bytes)};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(
        exhausted("grouped shuffle cancellation encoding allocation failed"));
  }
}
...
decode_distributed_vector_grouped_aggregate_shuffle_job_control_request_v3_exact(
  ...
  if (bytes.size() != kFrameLength || bytes.size() > limits.maximum_frame_length ||
    ...
    return common::make_unexpected(corruption("grouped shuffle cancellation framing is invalid"));
  }
  ...
  if (!header_crc.has_value() || !frame_crc.has_value() ||
    ...
    return common::make_unexpected(corruption("grouped shuffle cancellation checksum differs"));
  }
  ...
  if (!major.has_value() || !minor.has_value() || !header_length.has_value() ||
      ...
      !reserved.has_value()) {
    return common::make_unexpected(corruption("grouped shuffle cancellation is truncated"));
  }
  if (*major != kMajor || *minor != kMinor)
    return common::make_unexpected(
        unsupported("grouped shuffle cancellation version is unsupported"));
  DistributedVectorGroupedAggregateShuffleJobCancel request{
      .query_id = *query_id, .coordinator_node_id = *coordinator, .target_node_id = *target};
  if (*header_length != kHeaderLength || *frame_length != kFrameLength ||
      *action != static_cast<std::uint8_t>(
                     DistributedVectorGroupedAggregateShuffleJobControlAction::kCancel) ||
      !all_zero(*action_reserved) || !all_zero(*reserved) || !validate_cancel(request).is_ok()) {
    return common::make_unexpected(corruption("grouped shuffle cancellation is noncanonical"));
  }
...
encode_distributed_vector_grouped_aggregate_shuffle_job_control_response_v3(
  ...
  if (response.action != DistributedVectorGroupedAggregateShuffleJobControlAction::kCancel ||
    ...
    return common::make_unexpected(invalid("grouped shuffle cancellation response is invalid"));
  }
  ...
  try {
    ...
    if (!write.is_ok() || !writer.full())
      return common::make_unexpected(
          invalid("grouped shuffle cancellation response layout failed"));
    return EncodedDistributedVectorGroupedAggregateShuffleJobControlResponse{std::move(bytes)};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(
        exhausted("grouped shuffle cancellation response allocation failed"));
  }
}
...
decode_distributed_vector_grouped_aggregate_shuffle_job_control_response_v3_exact(
  ...
  if (bytes.size() != kResponseFrameLength ||
    ...
    return common::make_unexpected(
        corruption("grouped shuffle cancellation response framing is invalid"));
  }
  ...
  if (!header_crc.has_value() || !frame_crc.has_value() ||
      ...
      *frame_crc != common::crc32c(bytes.first(bytes.size() - kTrailerLength))) {
    return common::make_unexpected(
        corruption("grouped shuffle cancellation response checksum differs"));
  }
  common::ByteReader reader{bytes};
  ...
  if (!major.has_value() || !minor.has_value() || !header_length.has_value() ||
    ...
    return common::make_unexpected(
        corruption("grouped shuffle cancellation response is truncated"));
  }
  if (*major != kMajor || *minor != kMinor)
    return common::make_unexpected(
        unsupported("grouped shuffle cancellation response version is unsupported"));
  ...
  if (*header_length != kResponseHeaderLength || *frame_length != kResponseFrameLength ||
    ...
    return common::make_unexpected(
        corruption("grouped shuffle cancellation response is noncanonical"));
  }
