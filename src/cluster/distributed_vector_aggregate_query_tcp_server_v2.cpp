...
[[nodiscard]] common::Status poll_error(const int error = errno) {
  return {common::StatusCode::kIoError,
          std::string("polling vector aggregate query v2 TCP server: ") +
              std::error_code(error, std::generic_category()).message()};
}

[[nodiscard]] bool valid_timeout(const std::chrono::milliseconds timeout) noexcept {
  const auto maximum = std::chrono::duration_cast<std::chrono::milliseconds>(
      DistributedVectorAggregateQueryTlsServerV2::TimePoint::duration::max());
...
class DistributedVectorAggregateQueryTcpServerV2::Impl {
public:
  struct Connection {
    Connection(network::TcpSocket owned_socket,
               DistributedVectorAggregateQueryTlsServerV2 owned_carrier) noexcept
    ...
    DistributedVectorAggregateQueryTlsServerV2 carrier;
  };

  Impl(DistributedVectorAggregateQueryTcpServerConfigV2 configured,
  ...
  void accept_ready(const std::chrono::steady_clock::time_point now) {
    for (std::size_t admitted = 0U; admitted < config.maximum_accepts_per_poll; ++admitted) {
      ...
      auto carrier =
          DistributedVectorAggregateQueryTlsServerV2::create(std::move(*tls),
  ...
  DistributedVectorAggregateQueryTcpServerConfigV2 config;
  network::TcpListener listener;
  network::TlsServerContext tls_context;
  std::vector<std::unique_ptr<Connection>> connections;
  std::vector<pollfd> poll_descriptors;
  DistributedVectorAggregateQueryTcpServerMetricsV2 server_metrics;
  bool running{true};
};

DistributedVectorAggregateQueryTcpServerV2::DistributedVectorAggregateQueryTcpServerV2() noexcept =
    default;
DistributedVectorAggregateQueryTcpServerV2::DistributedVectorAggregateQueryTcpServerV2(
    std::unique_ptr<Impl> implementation) noexcept
    : implementation_(std::move(implementation)) {}
DistributedVectorAggregateQueryTcpServerV2::~DistributedVectorAggregateQueryTcpServerV2() = default;
DistributedVectorAggregateQueryTcpServerV2::DistributedVectorAggregateQueryTcpServerV2(
    DistributedVectorAggregateQueryTcpServerV2&&) noexcept = default;
DistributedVectorAggregateQueryTcpServerV2& DistributedVectorAggregateQueryTcpServerV2::operator=(
    DistributedVectorAggregateQueryTcpServerV2&&) noexcept = default;

common::Result<DistributedVectorAggregateQueryTcpServerV2>
DistributedVectorAggregateQueryTcpServerV2::start(
    DistributedVectorAggregateQueryTcpServerConfigV2 config) {
  if (config.authenticator == nullptr || config.receiver == nullptr ||
    ...
    return common::make_unexpected(
        invalid("vector aggregate query v2 TCP server configuration is invalid"));
  }
  ...
  try {
    ...
    return DistributedVectorAggregateQueryTcpServerV2{
        std::make_unique<Impl>(std::move(config), std::move(*listener), std::move(*context),
                               std::move(connections), std::move(descriptors))};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(
        exhausted("vector aggregate query v2 TCP server allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(
        exhausted("vector aggregate query v2 TCP server limits are too large"));
  }
}

common::Status DistributedVectorAggregateQueryTcpServerV2::poll_once(
    const std::chrono::milliseconds maximum_wait) {
  if (!implementation_ || !implementation_->running)
    return invalid("vector aggregate query v2 TCP server is not running");
  ...
  for (std::size_t remaining = impl.connections.size(); remaining > 0U; --remaining) {
    ...
    if (!status.is_ok() || state == DistributedVectorAggregateQueryTlsStateV2::kFailed)
      impl.remove_connection(index, false);
    else if (state == DistributedVectorAggregateQueryTlsStateV2::kComplete)
...
common::Status DistributedVectorAggregateQueryTcpServerV2::shutdown() {
  if (!implementation_ || !implementation_->running)
    return common::Status::ok();
  Impl& impl = *implementation_;
...
network::Ipv4Endpoint DistributedVectorAggregateQueryTcpServerV2::bound_endpoint() const noexcept {
  return implementation_ ? implementation_->listener.bound_endpoint() : network::Ipv4Endpoint{};
}

DistributedVectorAggregateQueryTcpServerMetricsV2
DistributedVectorAggregateQueryTcpServerV2::metrics() const noexcept {
  return implementation_ ? implementation_->server_metrics
                         : DistributedVectorAggregateQueryTcpServerMetricsV2{};
}

bool DistributedVectorAggregateQueryTcpServerV2::is_running() const noexcept {
