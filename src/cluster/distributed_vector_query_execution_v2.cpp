...
DistributedVectorQueryExecutionV2::DistributedVectorQueryExecutionV2(
...
common::Result<DistributedVectorQueryExecutionV2>
DistributedVectorQueryExecutionV2::create(const raft::NodeId source_node_id,
                                          query::CompatibleDistributedVectorSnapshotV2 snapshot,
                                          const DistributedVectorQueryExecutionLimitsV2 limits) {
  const auto dispatches = snapshot.dispatches();
  if (source_node_id == 0U || dispatches.empty())
    return common::make_unexpected(invalid("vector query v2 execution authority is invalid"));
  ...
  try {
    ...
    for (std::size_t index = 0U; index < dispatches.size(); ++index) {
      ...
      if (dispatch.query_id != query_id ||
        ...
        return common::make_unexpected(
            invalid("vector query v2 execution authority is mixed or duplicated"));
      }
    ...
    auto coordinator = DistributedVectorResultCoordinatorV2::create(
        query_id, std::move(tablets), snapshot.result_schema(), limits.coordinator);
    if (!coordinator.has_value())
      return common::make_unexpected(coordinator.error());
    return DistributedVectorQueryExecutionV2{std::move(snapshot), std::move(*coordinator),
                                             std::move(senders), std::move(indexes)};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(exhausted("vector query v2 execution allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(exhausted("vector query v2 execution exceeds limits"));
  }
}

common::Result<std::size_t>
DistributedVectorQueryExecutionV2::sender_index(const schema::TabletId& tablet_id) const {
  const auto found = sender_indexes_.find(tablet_id);
  if (found == sender_indexes_.end()) {
    return common::make_unexpected(
        invalid("vector query v2 execution tablet does not belong to the snapshot"));
  }
  return found->second;
}

common::Result<DistributedVectorQueryAttemptV2>
DistributedVectorQueryExecutionV2::begin_attempt(const schema::TabletId& tablet_id,
                                                 const TimePoint now) {
  if (finished_)
    return common::make_unexpected(invalid("vector query v2 execution is finished"));
...
common::Status DistributedVectorQueryExecutionV2::accept_responses(
    const schema::TabletId& tablet_id,
    const std::span<const DistributedVectorQueryResponseV2> responses, const TimePoint now) {
  if (finished_)
    return invalid("vector query v2 execution is finished");
  auto index = sender_index(tablet_id);
  if (!index.has_value())
    return index.error();
...
common::Status DistributedVectorQueryExecutionV2::record_transport_failure(
...
common::Status DistributedVectorQueryExecutionV2::publish_terminal_state(SenderSlot& slot) {
  if (slot.sender.state() == DistributedQuerySenderState::kSucceeded &&
    ...
    if (!slot.sender.result().has_value()) {
      const common::Status failure{common::StatusCode::kInternal,
                                   "vector query v2 sender success has no result stream"};
    ...
    const common::Status reported = coordinator_.worker_failed(
        slot.tablet_id, {code, "vector query v2 sender reached terminal failure"});
    if (!reported.is_ok())
      return reported;
    slot.coordinator_failure_delivered = true;
  }
...
DistributedVectorQueryExecutionV2::sender_state(const schema::TabletId& tablet_id) const {
  auto index = sender_index(tablet_id);
  if (!index.has_value())
    return common::make_unexpected(index.error());
...
common::Result<std::optional<DistributedVectorQueryExecutionV2::TimePoint>>
DistributedVectorQueryExecutionV2::next_attempt_not_before(
...
DistributedVectorQueryExecutionV2::suggested_leader(const schema::TabletId& tablet_id) const {
  auto index = sender_index(tablet_id);
  if (!index.has_value())
    return common::make_unexpected(index.error());
...
DistributedVectorQueryExecutionV2::finish() {
  if (finished_)
    return common::make_unexpected(invalid("vector query v2 execution is already finished"));
  ...
  for (const SenderSlot& slot : senders_) {
    if (slot.sender.state() == DistributedQuerySenderState::kFailed ||
      ...
      return common::make_unexpected(
          failed.has_value() ? common::Status{common::StatusCode::kInternal,
                                              "failed vector query v2 execution produced a result"}
                             : failed.error());
    }
  ...
  if (!all_succeeded)
    return common::make_unexpected(unavailable("vector query v2 execution is incomplete"));
  ...
  try {
    ...
    finished_ = true;
    return DistributedVectorQueryExecutionResultV2{.plan = std::move(plan),
                                                   .result = std::move(*result)};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(exhausted("vector query v2 execution result allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(exhausted("vector query v2 execution result exceeds limits"));
  }
}

const query::CompatibleDistributedVectorSnapshotV2&
DistributedVectorQueryExecutionV2::snapshot() const noexcept {
  return snapshot_;
}
