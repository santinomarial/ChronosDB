...
[[nodiscard]] common::Status poll_error(const int error = errno) {
  return {common::StatusCode::kIoError,
          std::string("polling vector grouped aggregate query v2 TCP server: ") +
              std::error_code(error, std::generic_category()).message()};
}

[[nodiscard]] bool valid_timeout(const std::chrono::milliseconds timeout) noexcept {
  const auto maximum = std::chrono::duration_cast<std::chrono::milliseconds>(
      DistributedVectorGroupedAggregateQueryTlsServerV2::TimePoint::duration::max());
...
class DistributedVectorGroupedAggregateQueryTcpServerV2::Impl {
public:
  struct Connection {
    Connection(network::TcpSocket owned_socket,
               DistributedVectorGroupedAggregateQueryTlsServerV2 owned_carrier) noexcept
    ...
    DistributedVectorGroupedAggregateQueryTlsServerV2 carrier;
  };

  Impl(DistributedVectorGroupedAggregateQueryTcpServerConfigV2 configured,
  ...
  void accept_ready(const std::chrono::steady_clock::time_point now) {
    for (std::size_t admitted = 0U; admitted < config.maximum_accepts_per_poll; ++admitted) {
      ...
      auto carrier = DistributedVectorGroupedAggregateQueryTlsServerV2::create(
          std::move(*tls),
  ...
  DistributedVectorGroupedAggregateQueryTcpServerConfigV2 config;
  network::TcpListener listener;
  network::TlsServerContext tls_context;
  std::vector<std::unique_ptr<Connection>> connections;
  std::vector<pollfd> poll_descriptors;
  DistributedVectorGroupedAggregateQueryTcpServerMetricsV2 server_metrics;
  bool running{true};
};

DistributedVectorGroupedAggregateQueryTcpServerV2::
    DistributedVectorGroupedAggregateQueryTcpServerV2() noexcept = default;
DistributedVectorGroupedAggregateQueryTcpServerV2::
    DistributedVectorGroupedAggregateQueryTcpServerV2(std::unique_ptr<Impl> implementation) noexcept
    : implementation_(std::move(implementation)) {}
DistributedVectorGroupedAggregateQueryTcpServerV2::
    ~DistributedVectorGroupedAggregateQueryTcpServerV2() = default;
DistributedVectorGroupedAggregateQueryTcpServerV2::
    DistributedVectorGroupedAggregateQueryTcpServerV2(
        DistributedVectorGroupedAggregateQueryTcpServerV2&&) noexcept = default;
DistributedVectorGroupedAggregateQueryTcpServerV2&
DistributedVectorGroupedAggregateQueryTcpServerV2::operator=(
    DistributedVectorGroupedAggregateQueryTcpServerV2&&) noexcept = default;

common::Result<DistributedVectorGroupedAggregateQueryTcpServerV2>
DistributedVectorGroupedAggregateQueryTcpServerV2::start(
    DistributedVectorGroupedAggregateQueryTcpServerConfigV2 config) {
  if (config.authenticator == nullptr || config.receiver == nullptr ||
    ...
    return common::make_unexpected(
        invalid("vector grouped aggregate query v2 TCP server configuration is invalid"));
  }
  ...
  try {
    ...
    return DistributedVectorGroupedAggregateQueryTcpServerV2{
        std::make_unique<Impl>(std::move(config), std::move(*listener), std::move(*context),
                               std::move(connections), std::move(descriptors))};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(
        exhausted("vector grouped aggregate query v2 TCP server allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(
        exhausted("vector grouped aggregate query v2 TCP server limits are too large"));
  }
}

common::Status DistributedVectorGroupedAggregateQueryTcpServerV2::poll_once(
    const std::chrono::milliseconds maximum_wait) {
  if (!implementation_ || !implementation_->running)
    return invalid("vector grouped aggregate query v2 TCP server is not running");
  if (maximum_wait.count() < 0 || maximum_wait.count() > INT_MAX)
    return invalid("vector grouped aggregate query v2 TCP poll timeout is invalid");
  ...
  for (std::size_t remaining = impl.connections.size(); remaining > 0U; --remaining) {
    ...
    if (!status.is_ok() || state == DistributedVectorGroupedAggregateQueryTlsStateV2::kFailed)
      impl.remove_connection(index, false);
    else if (state == DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete)
...
common::Status DistributedVectorGroupedAggregateQueryTcpServerV2::shutdown() {
  if (!implementation_ || !implementation_->running)
    return common::Status::ok();
  Impl& impl = *implementation_;
...
DistributedVectorGroupedAggregateQueryTcpServerV2::bound_endpoint() const noexcept {
  return implementation_ ? implementation_->listener.bound_endpoint() : network::Ipv4Endpoint{};
}

DistributedVectorGroupedAggregateQueryTcpServerMetricsV2
DistributedVectorGroupedAggregateQueryTcpServerV2::metrics() const noexcept {
  return implementation_ ? implementation_->server_metrics
                         : DistributedVectorGroupedAggregateQueryTcpServerMetricsV2{};
}

bool DistributedVectorGroupedAggregateQueryTcpServerV2::is_running() const noexcept {
