...
[[nodiscard]] common::Status poll_error(const int error = errno) {
  return {common::StatusCode::kIoError,
          std::string("polling distributed query TCP server: ") +
...
class DistributedQueryTcpServer::Impl {
public:
  struct Connection {
    Connection(network::TcpSocket owned_socket, DistributedQueryTlsServer owned_carrier) noexcept
    ...
    DistributedQueryTlsServer carrier;
  };

  Impl(DistributedQueryTcpServerConfig configured, network::TcpListener listener_owner,
  ...
  void accept_ready(const std::chrono::steady_clock::time_point now) {
    for (std::size_t admitted = 0U; admitted < config.maximum_accepts_per_poll; ++admitted) {
      ...
      auto carrier = DistributedQueryTlsServer::create(std::move(*tls),
  ...
  DistributedQueryTcpServerConfig config;
  network::TcpListener listener;
  network::TlsServerContext tls_context;
  std::vector<std::unique_ptr<Connection>> connections;
  std::vector<pollfd> poll_descriptors;
  DistributedQueryTcpServerMetrics server_metrics;
  bool running{true};
};

DistributedQueryTcpServer::DistributedQueryTcpServer() noexcept = default;
DistributedQueryTcpServer::DistributedQueryTcpServer(std::unique_ptr<Impl> implementation) noexcept
    : implementation_(std::move(implementation)) {}
DistributedQueryTcpServer::~DistributedQueryTcpServer() = default;
DistributedQueryTcpServer::DistributedQueryTcpServer(DistributedQueryTcpServer&&) noexcept =
    default;
DistributedQueryTcpServer&
DistributedQueryTcpServer::operator=(DistributedQueryTcpServer&&) noexcept = default;

common::Result<DistributedQueryTcpServer>
DistributedQueryTcpServer::start(DistributedQueryTcpServerConfig config) {
  if (config.authenticator == nullptr || config.receiver == nullptr ||
    ...
    return common::make_unexpected(
        invalid("distributed query TCP server configuration is invalid"));
  }
  ...
  try {
    ...
    return DistributedQueryTcpServer{
        std::make_unique<Impl>(std::move(config), std::move(*listener), std::move(*context),
                               std::move(connections), std::move(descriptors))};
  } catch (const std::bad_alloc&) {
    return common::make_unexpected(exhausted("distributed query TCP server allocation failed"));
  } catch (const std::length_error&) {
    return common::make_unexpected(exhausted("distributed query TCP server limits are too large"));
  }
}

common::Status DistributedQueryTcpServer::poll_once(const std::chrono::milliseconds maximum_wait) {
  if (!implementation_ || !implementation_->running)
    return invalid("distributed query TCP server is not running");
  if (maximum_wait.count() < 0 || maximum_wait.count() > INT_MAX)
    return invalid("distributed query TCP poll timeout is invalid");
  ...
  for (std::size_t remaining = impl.connections.size(); remaining > 0U; --remaining) {
    ...
    if (!status.is_ok() || state == DistributedQueryTlsServerState::kFailed)
      impl.remove_connection(index, false);
    else if (state == DistributedQueryTlsServerState::kComplete)
...
common::Status DistributedQueryTcpServer::shutdown() {
  if (!implementation_ || !implementation_->running)
    return common::Status::ok();
  Impl& impl = *implementation_;
...
network::Ipv4Endpoint DistributedQueryTcpServer::bound_endpoint() const noexcept {
  return implementation_ ? implementation_->listener.bound_endpoint() : network::Ipv4Endpoint{};
}

DistributedQueryTcpServerMetrics DistributedQueryTcpServer::metrics() const noexcept {
  return implementation_ ? implementation_->server_metrics : DistributedQueryTcpServerMetrics{};
}

bool DistributedQueryTcpServer::is_running() const noexcept {
