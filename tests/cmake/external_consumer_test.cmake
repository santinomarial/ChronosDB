...
int main() {
  ...
  const auto create_replicated_follower_distributed_grouped_query =
      &chronos::service::create_replicated_follower_distributed_grouped_float64_query;
  ...
  const auto grouped_result_direction =
  ...
  const auto create_vector_aggregate_query_execution =
  ...
  const auto create_vector_aggregate_query_tls_server =
  ...
  using ConsumeVectorQueryRequest = chronos::common::Result<
      chronos::cluster::DistributedVectorQueryRequestReadStep> (
      chronos::cluster::DistributedVectorQueryRequestReader::*)(chronos::common::ByteView);
  const ConsumeVectorQueryRequest consume_vector_query_request =
      &chronos::cluster::DistributedVectorQueryRequestReader::consume;
  using ConsumeVectorQueryResponse = chronos::common::Result<
      chronos::cluster::DistributedVectorQueryResponseReadStep> (
      chronos::cluster::DistributedVectorQueryResponseReader::*)(chronos::common::ByteView);
  const ConsumeVectorQueryResponse consume_vector_query_response =
  ...
  const auto create_mutable_vector_query_execution =
      &chronos::cluster::DistributedMutableVectorQueryExecution::create;
  const auto describe_mutable_vector_query_identity =
  ...
  const auto request_native_query_cancel =
      &chronos::service::NativeQueryCancellation::request_cancel;
  const auto observe_native_query_cancel =
      &chronos::service::NativeQueryCancellation::requested;
  const auto acquire_mutable_worker_context =
      &chronos::service::ReplicatedIngestDatabase::acquire;
  const auto lookup_replicated_cluster_node =
      &chronos::service::ReplicatedQuerySnapshot::cluster_node;
  ...
  const auto create_replicated_follower_vector_aggregate_query_v2 =
      &chronos::service::create_replicated_follower_distributed_vector_aggregate_query_v2;
  const auto create_replicated_follower_vector_aggregate_lifecycle_v2 =
  ...
  const auto start_raft_observation_tcp_server =
      &chronos::cluster::RaftObservationTcpServer::start;
  ...
  (void)describe_mutable_vector_query_identity;
  ...
  (void)request_native_query_cancel;
  (void)observe_native_query_cancel;
  (void)acquire_mutable_worker_context;
  (void)lookup_replicated_cluster_node;
  ...
  (void)create_replicated_follower_vector_aggregate_query_v2;
  ...
  const auto encode_grouped_terminal =
      &chronos::query::encode_grouped_exchange_terminal_message;
  const auto decode_grouped_terminal =
      &chronos::query::decode_grouped_exchange_terminal_message_exact;
  const auto consume_grouped_terminal =
  ...
  const auto encode_distributed_grouped_fragment_dispatch =
      &chronos::query::encode_distributed_grouped_float64_fragment_dispatch;
  const auto decode_distributed_grouped_fragment_dispatch =
  ...
  const auto bind_distributed_grouped_fragment_dispatch =
      &chronos::query::bind_distributed_grouped_float64_fragment_dispatch;
  const auto encode_distributed_grouped_query_request =
  ...
  const auto create_distributed_grouped_query_execution =
      &chronos::cluster::DistributedGroupedQueryExecution::create;
  const auto create_distributed_grouped_query_tcp_execution =
      &chronos::cluster::DistributedGroupedQueryTcpExecution::create;
  ...
  const auto start_distributed_grouped_query_tcp_server =
      &chronos::cluster::DistributedGroupedQueryTcpServer::start;
  ...
  return installed_log.has_value() && installed_identity.has_value() &&
                 ...
                 encode_distributed_grouped_fragment_dispatch != nullptr &&
                 decode_distributed_grouped_fragment_dispatch != nullptr &&
                 ...
                 create_distributed_grouped_query_execution != nullptr &&
                 create_distributed_grouped_query_tcp_execution != nullptr &&
                 ...
                 describe_mutable_vector_query_identity != nullptr &&
