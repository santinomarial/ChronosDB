...
TEST(DistributedVectorGroupedAggregateShuffleTcpServerAllocationFailureTest,
  ...
  for (std::size_t fail_after = 0U; fail_after < 256U; ++fail_after) {
    ...
    auto result = run_failure(fail_after, [&] {
      return DistributedVectorGroupedAggregateShuffleTcpServer::start(std::move(config));
    });
    if (!result.has_value()) {
      saw_failure = true;
