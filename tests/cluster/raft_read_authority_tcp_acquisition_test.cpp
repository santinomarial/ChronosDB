...
[[nodiscard]] RaftReadAuthority authority() {
  return {
      .barrier = {.group_id = group(), .barrier = {.term = 6U, .context = 10U, .read_index = 12U}},
...
TEST(RaftReadAuthorityTcpAcquisitionTest, RotatesAddressesWithinOneImmutableRequestBudget) {
  ...
  auto server = RaftReadAuthorityTcpServer::start(
      {.listener = {},
  ...
  auto acquisition = RaftReadAuthorityTcpAcquisition::create(acquisition_config(
  ...
  for (std::size_t iteration = 0U;
       iteration < 4096U && acquisition->state() == RaftReadAuthorityTcpAcquisitionState::kRunning;
       ++iteration) {
  ...
  ASSERT_EQ(acquisition->state(), RaftReadAuthorityTcpAcquisitionState::kComplete)
...
TEST(RaftReadAuthorityTcpAcquisitionTest, RejectsAmbiguousRoutesAndCancelsActiveAttempt) {
  Authorizer authorizer;
  ...
  EXPECT_EQ(RaftReadAuthorityTcpAcquisition::create(std::move(duplicate)).error().code(),
            common::StatusCode::kInvalidArgument);
  auto wrong_node = acquisition_config({endpoint}, *tls_context, authenticator, authorizer);
  wrong_node.route.node_id = 3U;
  EXPECT_EQ(RaftReadAuthorityTcpAcquisition::create(std::move(wrong_node)).error().code(),
            common::StatusCode::kInvalidArgument);

  auto listener = network::TcpListener::bind();
  ASSERT_TRUE(listener.has_value());
  auto acquisition = RaftReadAuthorityTcpAcquisition::create(
  ...
  EXPECT_EQ(acquisition->state(), RaftReadAuthorityTcpAcquisitionState::kCancelled);
