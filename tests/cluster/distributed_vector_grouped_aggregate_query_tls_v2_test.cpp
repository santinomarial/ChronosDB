...
struct Carriers {
  ...
  SocketPair sockets;
  DistributedVectorGroupedAggregateQueryTlsServerV2 server;
  DistributedVectorGroupedAggregateQueryTlsClientV2 client;
};
...
struct CarrierSetup {
  ...
  DistributedVectorGroupedAggregateQueryTlsLimitsV2 server_limits;
  DistributedVectorGroupedAggregateQueryTlsLimitsV2 client_limits;
};
...
[[nodiscard]] common::Result<Carriers> create_carriers(const CarrierSetup& setup,
  ...
  const auto start = DistributedVectorGroupedAggregateQueryTlsClientV2::TimePoint{};
  auto server = DistributedVectorGroupedAggregateQueryTlsServerV2::create(
  ...
  auto client = DistributedVectorGroupedAggregateQueryTlsClientV2::create(
...
TEST(DistributedVectorGroupedAggregateQueryTlsV2Test,
  ...
  const DistributedVectorGroupedAggregateQueryTlsLimitsV2 limits{
  ...
  const auto progress_time =
      DistributedVectorGroupedAggregateQueryTlsClientV2::TimePoint{} + std::chrono::milliseconds{1};
  for (std::size_t iteration = 0U; iteration < 4096U; ++iteration) {
    ...
    if (carriers->client.state() == DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete &&
        carriers->server.state() == DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete) {
      break;
    }
  }
  ASSERT_EQ(carriers->client.state(), DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete);
  ASSERT_EQ(carriers->server.state(), DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete);
...
TEST(DistributedVectorGroupedAggregateQueryTlsV2Test,
  ...
  auto expected_aggregates = aggregates();
  auto resources = query::QueryResourceContext::create(1U << 20U).value();
  DistributedVectorGroupedAggregateQueryTlsClientConfigV2 config{
  ...
  EXPECT_EQ(client->state(), DistributedVectorGroupedAggregateQueryTlsStateV2::kFailed);
  ...
  EXPECT_EQ(DistributedVectorGroupedAggregateQueryTlsClientV2::create(
...
TEST(DistributedVectorGroupedAggregateQueryTlsV2Test,
  ...
  EXPECT_EQ(carriers->client.state(), DistributedVectorGroupedAggregateQueryTlsStateV2::kFailed);
  EXPECT_EQ(carriers->client.failure().code(), common::StatusCode::kUnauthenticated);
  EXPECT_EQ(carriers->server.state(),
            DistributedVectorGroupedAggregateQueryTlsStateV2::kReadingRequest);
  EXPECT_EQ(worker.bind_calls, 0U);
  EXPECT_EQ(worker.execute_calls, 0U);
}

TEST(DistributedVectorGroupedAggregateQueryTlsV2Test, ClearsIncompletePrefixAboveClientByteBudget) {
  Authorizer authorizer;
  Worker worker;
  auto receiver = DistributedVectorGroupedAggregateQueryReceiverV2::create(
      {.local_node_id = 2U, .authorizer = &authorizer, .worker = &worker});
  ...
  Authenticator client_authenticator{91U};
  Authenticator server_authenticator{92U};
  const DistributedVectorGroupedAggregateQueryTlsLimitsV2 server_limits{
      .handshake_timeout = std::chrono::milliseconds{100},
      .exchange_timeout = std::chrono::milliseconds{100},
      .maximum_response_frames = 2U,
      .maximum_response_bytes = 4096U};
