...
TEST(DistributedVectorGroupedAggregateQueryTcpClientV2Test,
  ...
  std::optional<DistributedVectorGroupedAggregateQueryTlsServerV2> server;
  for (std::size_t iteration = 0U; iteration < 4096U; ++iteration) {
      ...
      if (accepted->has_value()) {
        ...
        auto carrier = DistributedVectorGroupedAggregateQueryTlsServerV2::create(
            ...
            {.authenticator = &client_authenticator,
             ...
             .limits = limits()},
            DistributedVectorGroupedAggregateQueryTlsServerV2::TimePoint::clock::now());
    ...
    const auto now = DistributedVectorGroupedAggregateQueryTcpClientV2::TimePoint::clock::now();
    ASSERT_TRUE(client
    ...
    if (client->state() == DistributedVectorGroupedAggregateQueryTcpClientStateV2::kComplete &&
        server.has_value() &&
        server->state() == DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete) {
      break;
    }
  }
  ...
  EXPECT_EQ(server->state(), DistributedVectorGroupedAggregateQueryTlsStateV2::kComplete);
...
TEST(DistributedVectorGroupedAggregateQueryTcpClientV2Test,
  ...
  ASSERT_TRUE(request.has_value());
  ASSERT_TRUE(resources.has_value());
  const auto start = DistributedVectorGroupedAggregateQueryTcpClientV2::TimePoint{};
  auto config = DistributedVectorGroupedAggregateQueryTcpClientConfigV2{
      .remote_endpoint = listener->bound_endpoint(),
  ...
  EXPECT_EQ(client->state(), DistributedVectorGroupedAggregateQueryTcpClientStateV2::kFailed);
  ...
  auto invalid = DistributedVectorGroupedAggregateQueryTcpClientV2::begin(
  ...
  auto mismatched = DistributedVectorGroupedAggregateQueryTcpClientV2::begin(
