...
TEST(NetworkSecurityTest, PlaintextIsRestrictedToLoopback) {
  ...
  EXPECT_TRUE(authenticate_peer(config, {{127U, 1U, 2U, 3U}, false}).has_value());
  EXPECT_FALSE(authenticate_peer(config, {{10U, 0U, 0U, 1U}, false}).has_value());
}
...
TEST(NetworkSecurityTest, CustomAuthenticatorAttachesOnlyNonzeroAuthorizedPrincipal) {
  ...
  const auto result = authenticate_peer(config, {{127U, 0U, 0U, 1U}, false});
  ...
  EXPECT_FALSE(authenticate_peer(config, {{127U, 0U, 0U, 1U}, false}).has_value());
  FixedAuthenticator denied{{.authorized = false}};
  config.authenticator = &denied;
  const auto rejection = authenticate_peer(config, {{127U, 0U, 0U, 1U}, false});
  ASSERT_TRUE(rejection.has_value());
  EXPECT_FALSE(rejection->authorized);
}

TEST(NetworkSecurityTest, TlsRequiredFailsClosedWithoutBackend) {
  ...
  const auto peer = authenticate_peer(config, {{127U, 0U, 0U, 1U}, false});
