...
[[nodiscard]] ColdPartDescriptor descriptor(const std::uint8_t seed,
  ...
  return ColdPartDescriptor{
      manifest::PartDescriptor{id<cseg::PartId>(seed),
                               id<schema::TableId>(10U),
                               id<schema::TabletId>(11U),
                               id<schema::SchemaId>(12U),
                               ...
                               0,
                               1},
      "parts/" + std::to_string(seed),
      ingest::sha256(bytes).value()};
}

TEST(TieredPartManagerTest, VerifiesUploadBeforeManifestInstallAndCachesFullObject) {
  MemoryObjectStore store;
  auto manager = TieredPartManager::create(
      store, TieringLimits{8U, 1024U, 6U, 1U});
  ...
  auto receipt = manager->upload_and_install(descriptor(1U, first), first,
  ...
  auto second_receipt = manager->upload_and_install(
      descriptor(2U, second), second,
