...
[[nodiscard]] VectorExpression text_predicate_expression() {
  ...
  instructions.emplace_back(VectorUnaryExpression{.operation = VectorUnaryOperation::kUpperAscii,
                                                  .operand_instruction = 0U});
  instructions.emplace_back(VectorConstantExpression{ScalarValue::text(string, "ABC").value()});
...
TEST(ColumnOutputAllocationFailureTest,
  ...
  for (std::size_t fail_after = 0U; fail_after < 96U; ++fail_after) {
    ...
    positions.emplace_back(ComputedColumnOutputPosition{text_predicate_expression()});
