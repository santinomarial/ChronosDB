...
Manifest v1 and existing v1 part validators remain unchanged. Strict v2 metadata entry points now
encode and decode the expanded registry, canonical directories, checksums, runtime limits, and
schema binding without weakening the v1 entry points. Manifest v2 must explicitly admit CSEG v2
and carry source/checkpoint meaning before v2 files are installed. Until then, metadata codec
support does not claim durable database publication.
...
confusion, and bind the exact table schema. Golden fixtures, hostile corruption matrices, full page
validation, Manifest v2 installation, current/as-of winner resolution, crash tests, fuzzing, and
performance evidence remain subsequent work and Phase 18 validation.
