...
fixtures remain valid. A later generation owner may store both encodings only with explicit nested-
format dispatch and must compose reference recovery with the exact durable chunk session before
...
and full byte validation. Durable generation integration, mixed old/new recovery, chunk/checkpoint
crash ordering, allocation failure, fuzzing, and large-transfer testing remain follow-up work.

## Migration and rollback

This format is additive. Rollback retains v1 self-contained checkpoint support but cannot resume a
reference-only generation. A future durable generation integration must define selection so older
readers never select bytes they can misinterpret.
