...
heartbeats, current-term read barriers, application-frontier advancement, and live-state node
crash/restart after valid setup traces. Exact recovered per-node images now permit direct restart
schedules at the terminal term and rejection at the reserved next proposal index without generating
impractically long prefixes. Broader exhaustive campaigns, clock changes, and physical log syscall
