...
the database/store identities and never move the base Manifest generation backward. At one base
generation routes are add-only; under [ADR 0191](0191-manifest-retirement-bound-cold-route-removal.md),
a newer base may omit only a predecessor route whose logical part disappeared, while every
still-logical route remains exact. Rekeying remains unauthorized. The implementation creates
...
temporary cleanup, same-base add-only transitions, Manifest-retirement-bound omission, wrong-base
rejection, highest-generation damage without fallback, and owner poisoning on an injected
post-rename directory-sync failure. Installed
...
- **Permit removal without a newer logical Manifest:** rejected because the current part could lose
  its only remote recovery route.
