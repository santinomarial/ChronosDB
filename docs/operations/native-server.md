...
shard latency. Do not raise a bound without measuring retained memory.

Shutdown closes every socket, detaches active work, and clears buffers. Stop shard response
production before destroying queues. TLS and authentication are separate integration boundaries;
do not expose plaintext transport outside a trusted development/test network.
