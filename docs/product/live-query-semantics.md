...
bytes/installation, topology transitions, state-retention defaults, window trigger cadence,
...
the Protocol 1.1 acknowledgement/checkpoint lifecycle, and exact logical coordinator restoration are
implemented. Every later choice must preserve [invariant 17](../architecture/invariants.md).
