...
> query chunks and optionally append the shared canonical row-version suffix. The aggregate Phase 9
> tablet factory now composes every published head with the durable subset for the current
> append-only operation set; future correction/delete row-version merging remains unimplemented. Retry
...
user columns into canonical query chunks and can opt into the shared row-version suffix. One such
scan is not complete tablet visibility. `create_snapshot_tablet_scan` composes all generations with
installed parts from the same aggregate publication for the current append-only operation set.
