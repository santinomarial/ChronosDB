...
A ten-case matrix recomputes CRC32C after independently changing magic, either version, layout,
filename-bound base identity, checkpoint range/count fields, or reserved bytes. Repeated recovery
preserves every image while distinguishing corruption from future-version incompatibility;
integrity coverage cannot replace semantic or compatibility validation.
