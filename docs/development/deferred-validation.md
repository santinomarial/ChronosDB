...
- Expand CSEG v2 projected-reader and semantic validation beyond the implemented canonical
  ...
  independently generated field-level bytes, broader checksum-repaired semantic page-body matrices
  beyond the implemented source/identity/position/operation cases, WAL/Raft source-lineage cases,
  allocation failure, fuzzing, cross-version conversion, and cross-compiler fixtures after the
  accepted temporal registry and layout planner.
