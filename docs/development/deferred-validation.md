...
- Extend the implemented committed-only tablet command application and full retained-log rebuild
  beyond its four-cut publication/applied-index process-crash matrix with deterministic applied-index
  persistence I/O failures and multi-entry schedules. Extend the implemented durable
