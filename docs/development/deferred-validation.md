...
- Extend the v1.1 snapshot membership checkpoint validation beyond the independent
  ...
  reopen matrix crosses RTAS cleanup failure before/after unlink with every Raft repair stage: size,
  truncate, file sync, and directory sync. Every failed attempt or open withholds success, releases
...
- Configured `chronosd` Linux subprocess execution in CI, daemon ingest over real sockets, remaining
  ...
  record and Raft corruption matrices remain deferred. A minimal one-byte final-tail case also
  proves packaged startup requires explicit repair authorization and preserves the suffix; other
  ...
  qualification remains deferred. A companion portable case retains the valid anchor, corrupts one
  covered segment-2 header field, and twice rejects that exact checksum failure without rewriting
  either file; other retained-record, truncation, and packaged anchored-history combinations remain
  deferred.
