...
- Configured `chronosd` Linux subprocess execution in CI, daemon ingest over real sockets, remaining
  ...
  preserves it with segment 2. A portable aggregate-owner case also removes the authoritative anchor
  after segment-1 reclamation, twice requires the exact absent-base corruption with ownership
