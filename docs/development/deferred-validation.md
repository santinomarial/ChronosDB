...
- Membership protocol, leader leases if ever proposed, timerfd/event-loop and durable-completion
  composition, disk-error behavior, and storage fault injection. A bounded generation-tagged timer
  scheduler now emits exact election/heartbeat actions and rejects stale completion rearming.
