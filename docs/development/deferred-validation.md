...
- Expand CSEG v2 projected-reader and semantic validation beyond the implemented canonical
  ...
  additional granule/page field-level bytes beyond the implemented independently generated
  header/system-registry fixture, broader checksum-repaired semantic page-body matrices beyond the
  implemented source/identity/position/operation cases, WAL/Raft source-lineage cases, allocation
  failure, fuzzing, cross-version conversion, and cross-compiler fixtures after the accepted
  temporal registry and layout planner.
