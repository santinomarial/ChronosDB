...
- **Feature-pass status:** `chronos_raft` implements a deterministic follower/candidate/leader core,
  ...
  elections, and live-state crash/restart faults after a valid setup trace, and reports replay-limit
  truncation without claiming completion. Simulations may begin from exact per-node recovered
  persistent images; a combined terminal term/index schedule proves proposals and post-restart
  elections fail without mutation. Broader exhaustive action campaigns, clock changes, physical-log
  faults, and the full exit evidence remain deferred.
