...
- **Feature-pass status:** the existing parser/binder/executor `FOR SYSTEM_TIME AS OF` path now has
  ...
  application snapshots, allocation-failure coverage beyond the canonical one-tablet/no-part
  Manifest startup path, complete query-epoch publication, v1 migration, direct vector winner
