...
- QUORUM_SYNC receipts, Protocol 2.0 negotiation/acknowledgement bytes, metadata-derived local
  ...
  replay with bounded terminal-only result ownership and an authenticated deadline-bound TCP/TLS
  redirect carrier, but its poll execution owner, packaged command, and authoritative server
  emission are still absent.
...
1. Add remote mutable-tablet query fragments, compose the implemented finite-query TCP client with
   poll execution and a packaged command, then extend the existing three-process
