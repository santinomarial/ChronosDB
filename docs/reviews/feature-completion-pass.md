...
- Distributed Native execution covers bounded row-backed multi-key/all-type grouping, global
  ...
  TCP-owned independent-process shuffle result scheduling, duplicate collection, and lifecycle
  composition, and
  ...
  and exact reverse-route success receipt composed through a bounded mutual-TLS carrier and finite
  byte-identical retry owner.
...
- Authenticated grouped reducer coordinator leases: additive fixed Job Control v4 frames activate
  ...
  clocks; the original PREPARE deadline remains the pre-activation and hard fallback bound. Real
  process gates prove that deadline after acknowledged PREPARE and route installation, and prove
  earlier lease expiry after activation plus renewal. Every case accepts a distinct fresh query.
