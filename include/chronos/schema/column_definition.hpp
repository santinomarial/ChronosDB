...
public:
  ...
  [[nodiscard]] static common::Result<ColumnDefinition> create(ColumnId id, std::string name,
                                                               LogicalType type, bool nullable);

  [[nodiscard]] constexpr const ColumnId& id() const noexcept {
    return id_;
  }
  [[nodiscard]] const std::string& name() const noexcept {
    return name_;
  }
  [[nodiscard]] constexpr const LogicalType& type() const noexcept {
    return type_;
  }
